...
const PostPage = () => {
  ...
  const filteredData = postData.filter((post) => {
    const matchesName = selectedPostName ? post.title === selectedPostName : true;
    const matchesCate = selectedCateId ? post.category.split(",")[1] === selectedCateId.toString() : true;
